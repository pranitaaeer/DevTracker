  ...
  return (
    <>
      <aside className="hidden lg:block fixed left-0 top-[72px] w-80 h-[calc(100vh-72px)] px-4 z-[100]">
        <div className="h-full rounded-2xl bg-white dark:bg-[#0d1117]  p-4 shadow-xl overflow-y-auto">
            ...
            {menu.map((item) => {
              ...
              return (
                <Link
                  key={item.name}
                  href={item.href}
                  className="group flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-zinc-600 dark:text-zinc-400 transition hover:bg-zinc-100 dark:hover:bg-zinc-800/70 hover:text-black dark:hover:text-white"
                >
                  <Icon
                    size={17}
