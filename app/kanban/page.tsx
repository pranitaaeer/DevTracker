...
export default function KanbanPage() {
  ...
  // function handleDelete(colId: string, cardId: string) {
  //   openConfirm({ title: 'Delete card', description: 'Are you sure?', onConfirm: () => { deleteCard(colId, cardId); addToast({ title: 'Card deleted', description: 'Undo', action: { label: 'Undo', onClick: () => { const restored = restoreCard(cardId, colId); if (restored) addToast({ title: 'Restored', description: restored.title }); } } }); } });
  // }
async function handleDelete(colId: string, cardId: string) {
  ...
  async function onCreate() {
    ...
    try {
      await addCard(colId, { title, description: desc, priority, dueDate: dueDate || undefined, projectId: linkedProject });
      addToast({ title: 'Card added' });
      setOpenNew(false); setTitle(''); setDesc(''); setPriority('medium'); setDueDate(''); setLinkedProject(undefined);
    } catch (e) {
  ...
  function onEdit(colIdParam: string, card: any) {
    ...
    setColId(colIdParam); setTitle(card.title); setDesc(card.description || ''); setPriority(card.priority || 'medium'); setDueDate(card.dueDate || ''); setLinkedProject(card.projectId || undefined); setOpenNew(true);
  }

  function onUpdate() {
    if (!editing) return;
    updateCard(editing.card.id, { title, description: desc, priority, dueDate: dueDate || undefined, projectId: linkedProject });
...
>New Card</button>
      ...
      <Card>
        <div className="overflow-auto">
          <KanbanBoard columns={kanban} onMove={handleMove} onDeleteCard={handleDelete} onEditCard={onEdit} />
        </div>
      </Card>
      ...
      <Dialog open={openNew} onClose={() => setOpenNew(false)} title={editing ? 'Edit Card' : 'New Card'}>
        <div className="grid gap-3">
          ...
          <select value={colId} onChange={(e) => setColId(e.target.value)} className="border text-black rounded px-3 py-2">
            {(kanban ?? []).map(c => <option key={c.id} value={c.id}>{c.title}</option>)}
          </select>
          ...
          <select value={linkedProject || ''} onChange={(e) => setLinkedProject(e.target.value || undefined)} className="border rounded px-3 py-2 text-black">
            ...
            {useDataStore.getState().projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
          </select>

          <div className="flex justify-end gap-2 mt-3">
            <button onClick={() => setOpenNew(false)} className="px-3 py-2 rounded">Cancel</button>
